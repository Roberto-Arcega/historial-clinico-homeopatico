...
function limpiarSeccion(seccionIndex) {
    ...
    if (confirm('¿ESTÁ SEGURA DE QUE DESEA LIMPIAR TODA LA INFORMACIÓN DE ESTA SECCIÓN?')) {
        ...
        if (seccion) {
            ...
            // Limpiar campos específicos por sección
// Función auxiliar para limpiar campos específicos por sección (CORREGIDA)
...
function generarHermanosNuevo() {
    ...
    for (let i = 1; i <= numero; i++) {
        ...
        div.innerHTML = `
            <div class="contenedor-familiar"> 
                <span class="etiqueta-familiar">Hermano ${i}:</span> 
                <div class="grupo-estado"> 
                    <label><input type="radio" name="estadoHermano${i}" value="Vivo" onchange="toggleDatosHermano(${i})" required> Vivo</label> 
                    <label><input type="radio" name="estadoHermano${i}" value="Muerto" onchange="toggleDatosHermano(${i})" required> Muerto</label> 
                </div> 
                <div class="grupo-datos" id="datosHermano${i}" style="display: none;"> 
                    <div class="campo-edad"> 
                        <label>Edad:</label> 
                        <select name="edadHermano${i}" class="select-edad"> 
                            <option value="">-</option> 
                            ${generarOpcionesEdad()} 
                        </select> 
                    </div> 
                </div> 
                <div class="grupo-muerte" id="causaMuerteHermano${i}" style="display: none;"> 
                    <div class="campo-causa-muerte"> 
                        <label>Causa de muerte:</label> 
                        <input type="text" name="causaMuerteHermano${i}" placeholder="Especificar causa..." class="input-causa-muerte"> 
                    </div> 
                </div> 
            </div>
        `;
